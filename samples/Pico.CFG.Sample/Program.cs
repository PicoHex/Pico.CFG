...
await Test("Basic key-value parsing", async () =>
{
...
await Test("Multiple source priority", async () =>
{
    var builder = CFG.CreateBuilder();
...
await Test("Multiple source types", async () =>
{
    var builder = CFG.CreateBuilder();
    ...
    return stringValue == "StringValue" && dictValue == "DictValue" && streamValue == "StreamValue";
});

// Test 4: Missing key handling
await Test("Missing key handling", async () =>
{
    var builder = CFG.CreateBuilder();
...
await Test("Complex key names", async () =>
{
    var builder = CFG.CreateBuilder();
...
await Test("Whitespace handling", async () =>
{
    var builder = CFG.CreateBuilder();
    builder.Add("\n\nKey1=Value1\n  \nKey2  =  Value2  \nKey3=\n\n");
...
await Test("Async cancellation", async () =>
{
    try
    {
        var builder = CFG.CreateBuilder();
...
await Test("Provider enumeration", async () =>
{
    var builder = CFG.CreateBuilder();
...
await Test("Change notification", async () =>
{
    var builder = CFG.CreateBuilder();
